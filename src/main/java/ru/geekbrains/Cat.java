...
public class Cat implements IActionable{
    String name;
    int maxRun;
    int maxJump;
    ...
    public void go(IBarrierable barrier) throws TooLongException {
        if (barrier instanceof Treadmill) {
            ...
            if (t.getLenght()>getMaxRun()) {
                throw new TooLongException(String.format("Threadmill lenght is %d, but cat limit is %d.", t.getLenght(), getMaxRun()));
            }
            else {
                run(t.getLenght());
            }
            ...
            if (w.getHeight()>getMaxJump()) {
                throw new TooLongException(String.format("Wall heaight is %d, but cat limit is %d.", w.getHeight(), getMaxJump()));
            }
            else {
                jump(w.getHeight());
            }
